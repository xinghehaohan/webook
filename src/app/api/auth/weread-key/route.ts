...
import { encryptWeReadKey, requestIsSameOrigin, securityConfigured, SESSION_COOKIE, sessionCookieOptions, UPGRADE_COOKIE, verifySession, WEREAD_KEY_COOKIE } from '@/lib/auth/session'
...
export async function POST(request: NextRequest) {
  ...
  if (process.env.NODE_ENV === 'production' && !verifySession(store.get(SESSION_COOKIE)?.value)) return fail('UNAUTHORIZED', '请先输入页边访问口令。', 401)
  let body: unknown
