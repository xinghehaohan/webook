...
export function ConnectionCard({ connection, onChanged }: { connection?: ConnectionState; onChanged: () => void }) {
  ...
  return <section className="connection-card">
    ...
    <div className="connection-copy"><span>微信读书连接</span><h3>{connected ? (connection?.authenticated ? '书架已准备好' : '需要解锁') : '等待你的 API Key'}</h3><p>{connected ? (connection?.configuredBy === 'device' ? 'API Key 已加密保存在这台设备的安全 Cookie 中。' : '正在使用服务器环境中的 API Key。') : '直接在下方粘贴 Key，验证成功后就会读取你的真实书架。'}</p></div>
