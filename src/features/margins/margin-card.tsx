...
export function MarginCard({ card, hero = false }: { card: MarginCardType; hero?: boolean }) {
  ...
  const rememberExploration = useReaderStore((state) => state.rememberExploration)
  const namespace = useReaderStore((state) => state.namespace)
  const pathname = usePathname()
  const favorite = Boolean(favorites[card.id])
  const href = `/opinion/${encodeURIComponent(card.book.bookId)}/${card.chapterUid}?range=${encodeURIComponent(card.range)}&bookmarkId=${encodeURIComponent(card.bookmarkId ?? card.id)}&from=${encodeURIComponent(pathname)}&cardId=${encodeURIComponent(card.id)}`
  const remember = () => {
    markSeen(card.id)
    rememberExploration(card)
  ...
  return <article id={`margin-${encodeURIComponent(card.id)}`} className={`margin-card${hero ? ' hero-card' : ''}`}>
